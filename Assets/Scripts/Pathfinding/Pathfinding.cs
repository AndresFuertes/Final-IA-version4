...
public class Pathfinding : MonoBehaviour
{
    ...
    private IEnumerator Path(Vector3 from, Vector3 to, Action<List<Node>> callback, Action errorCallback)
    {
        ...
        var actualRadius = 1.5f;
        var fromColliderArray = Physics.OverlapSphere(from, actualRadius, nodeMask);

        while (fromColliderArray.Length <= 0 && watchDogSearch > 0)
        {
            watchDogSearch--;
            actualRadius *= 2;
            fromColliderArray = Physics.OverlapSphere(from, actualRadius, nodeMask);
        }
        ...
        actualRadius = 1.5f;
        var toColliderArray = Physics.OverlapSphere(to, actualRadius, nodeMask);
        watchDogSearch = 500;
        ...
        while (toColliderArray.Length <= 0 && watchDogSearch > 0)
        {
            watchDogSearch--;
            actualRadius *= 2;
            toColliderArray = Physics.OverlapSphere(to, actualRadius, nodeMask);
        }
    ...
    {
        var actualRadius = 2;
        var colliders = Physics.OverlapSphere(point, actualRadius, LayerMask.GetMask("Node"));

        while (colliders.Length <= 0)
        {
            actualRadius *= 2;
            colliders = Physics.OverlapSphere(point, actualRadius, nodeMask);
        }
    ...
    public static bool FieldOfView(Transform from, Transform to, float viewAngle)
    {
        ...
        if (angle < viewAngle / 2 && HasLineOfSight(from.position, to.position))
        {
            // Dibujar el campo de visión en el editor
            UnityEngine.Debug.DrawLine(from.position, to.position, Color.green, 10f);
            return true;
        }

        // Dibujar el campo de visión en el editor si no hay visión
        UnityEngine.Debug.DrawLine(from.position, to.position, Color.red, 10f);
        return false;
    }
    ...
    {
        var dir = to - from;
        bool hasLineOfSight = !Physics.Raycast(from, dir, dir.magnitude, LayerMask.GetMask("Paredes", "Floor"));

        // Dibujar la línea de visión en el editor
        UnityEngine.Debug.DrawLine(from, to, hasLineOfSight ? Color.green : Color.red, 5f);
